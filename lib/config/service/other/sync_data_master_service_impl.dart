...
class SyncDataMasterServiceImpl implements SyncDataMasterService {
  ...
  Future<HttpResponse<SyncDataMasterModel>> syncDataMaster(
      {String? user_id, String? date, String? slug_database}) async {
    try {
      ...
      final formData = FormData.fromMap({'slug_database': slug_database});
      ...
      final result = await _dio.request('sync-data-master',
          data: formData, queryParameters: params, options: options);

      if (result.statusCode == HttpStatus.ok) {
        SyncDataMasterModel value =
            SyncDataMasterModel.fromJson(result.data!['data']);
